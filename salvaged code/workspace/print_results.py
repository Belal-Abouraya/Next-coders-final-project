...
def print_results(results_dic, results_stats_dic, model, 
    ...
    """    
    print("Used model architecture {}".format(model))
    ...
    Percentage of correct 'not a dog': {}
    Percentage of match: {}"""
    .format(results_stats_dic["pct_correct_dogs"],results_stats_dic["pct_correct_breed"], results_stats_dic["pct_correct_notdogs"],results_stats_dic["pct_match"]))
    
    if print_incorrect_dogs:
        if results_stats_dic["n_correct_dogs"] + results_stats_dic["n_correct_notdogs"] != results_stats_dic["n_images"]:
            for key in results_dic:
                if sum(results_dic[key][3:]) == 1:
                    print("Dog misclassification: " + key)
                    print(results_dic[key])
        else:
            print("No dog misclassifications")
    
    if print_incorrect_breed:
        if results_stats_dic["n_correct_dogs"] != results_stats_dic["n_correct_breed"]:
            for key in results_dic:
                if sum(results_dic[key][3:]) == 2 and results_dic[key][2] == 0:
                    print("Breed misclassification: " + key)
                    print(results_dic[key])
